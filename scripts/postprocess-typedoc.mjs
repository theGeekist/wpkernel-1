...
function transformLine(line, inCodeFence) {
	...
	if (!inCodeFence) {
		text = text.replace(/^(\s*#\s+Type Alias:\s+[^(]+)\(\)(.*)$/u, '$1$2');
	}
	...
	return { text: escaped.trimEnd(), inCodeFence };
}
...
async function processFile(filePath) {
	...
	try {
		for await (const line of rl) {
			const { text, inCodeFence: nextState } = transformLine(
				line,
				inCodeFence
			);
...
function resolveConcurrencyLimit() {
	...
	const parallelism = cpuCount > 0 ? cpuCount : 4;
