...
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| `Pipeline`          | A configured process-local evaluator of one compiled graph and one immutable configuration snapshot per run. It is neither durable host nor graph data.                | Public evaluator              |
...
| `Graph`             | The immutable compiled executable graph, including nodes, data edges, adjacency, canonical rank, projections and execution policy. It is the sole execution authority. | Compiler and scheduler        |
...
| `Edge`              | One directed data dependency from a node output to a dependant node. It implies readiness ordering because data must exist, but is not a generic precedence relation.  | Graph                         |
...
| `RunOutcome`        | The algebraic terminal result: succeeded, failed, cancelled or suspended, with complete immutable projections.                                                         | Run                           |
...
| `Anchor`            | Optional authoring reference compiled into graph structure. It is not an execution stage.                                                                              | Authoring                     |
...
| --------------------------------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `createPipeline`                                                                                    | Retain name, adapt semantics             | Creates a v2 `Pipeline`; exact options and builder syntax remain unresolved.                                                              |
| `makePipeline`                                                                                      | Remove from native v2                    | Compatibility factory for the v1 agnostic runtime only.                                                                                   |
| `makeResumablePipeline`                                                                             | Remove from native v2                    | Resume is a method or operation on `Pipeline` consuming `Suspension`.                                                                     |
...
| `PipelineDiagnostic`, `ConflictDiagnostic`, `MissingDependencyDiagnostic`, `UnusedHelperDiagnostic` | Adapt                                    | Split compile-time `GraphDiagnostic` from run-time `RunDiagnostic` and typed failure records.                                             |
...
| `PipelinePauseSnapshot`                                                                             | Rename                                   | Single-use `Suspension`, consumed by `resume` or compensating `abandon`, with private `Frontier`.                                         |
...
| `ResumablePipeline`                                                                                 | Fold into `Pipeline`                     | `resume(options)` consumes a process-local `Suspension`.                                                                                  |
...
| `CreatePipelineOptions`                                                                             | Adapt                                    | Options object for graph/configuration plus explicit middleware, observer and effect registries.                                          |
| `Pipeline`                                                                                          | Retain, redefine                         | The sole v2 configured evaluator.                                                                                                         |
| `ErrorFactory`                                                                                      | Adapt                                    | May construct typed boundary errors but cannot replace retained original graph failures.                                                  |
| `MaybePromise`                                                                                      | Retain                                   | Exactly `T \| PromiseLike<T>`; every callable `then` is adopted and only that observation promotes settlement.                            |
| `isPromiseLike`, `maybeAll`, `maybeThen`, `maybeTry`                                                | Retain only if generally useful and safe | Utilities must preserve v2 synchronous settlement; they carry no graph semantics.                                                         |
...
| `run`        | Evaluate a graph from new admitted input.                              |
