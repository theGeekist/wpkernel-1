...
A hiring portal that wires a `job` resource through the entire stack: REST controllers, React DataViews, and policy-aware actions. Follow the walkthrough in the [Showcase guide](/examples/showcase) to see how the kernel config drives the generated output.【F:examples/showcase/wpk.config.ts†L1-L340】
