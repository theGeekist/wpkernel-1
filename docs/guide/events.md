...
3.  **Action Start**: The `useAction` hook that powers the interactive store dispatches the WPKernel Action (e.g., `Job.Publish`). This immediately emits the `wpk.action.start` event.
...
7.  **Domain Event**: The Action emits a specific domain event, like `acme.job.updated`.
...
You can subscribe to events directly on the event bus for a fully-typed experience, or use the familiar `wp.hooks` API.
