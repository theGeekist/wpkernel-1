...
- **E2E selectors** – Components emit `data-wpk-*` attributes that power `@geekist/wp-kernel-e2e-utils` helpers.
...
The Jobs admin screen in `app/showcase/src/admin/pages/JobsList.tsx` is the canonical example. It pairs `ResourceDataView` with a `createDataFormController`-powered creation panel, emits `data-wpk-dataview-*` selectors, and demonstrates action gating via policies.
...
For end-to-end coverage, install `@geekist/wp-kernel-e2e-utils` and use the `kernel.dataview()` helpers:
...
- **Snapshot reference:** the Gutenberg sources live at `packages/ui/vendor/dataviews-snapshot/` for browsing only. Refresh them with `pnpm --filter @geekist/wp-kernel-ui update:dataviews-snapshot` when you need updated context.
...
- [Kernel package reference](./kernel.md)
