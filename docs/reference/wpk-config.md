...
| `schemas`                | Required record of schema descriptors keyed by identifier. Each descriptor has a `path` and may have a `description`. May be empty (`{}`), but the property itself must exist.                                  | Loaded into the IR and reused by PHP + UI builders.                                          | Schema metadata feeds REST argument metadata and resource type generation when a resource references a schema.                                      | Informs REST controller validation and resource type builders.                                                 | Schema provenance exposed on `ResourceObject` so clients can introspect which schema a resource uses.          |
