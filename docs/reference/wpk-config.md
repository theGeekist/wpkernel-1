...
| `schemas`                | Required record of schema descriptors keyed by identifier. Each descriptor has a `path` and may have a `description`. The record may be empty (`{}`), but the property itself must exist.                       | Loaded into the IR and reused by PHP + UI builders.                                          | Schema metadata feeds REST argument metadata and resource type generation when a resource references a schema.                                      | Informs REST controller validation and resource type builders.                                                 | Schema provenance exposed on `ResourceObject` so clients can introspect which schema a resource uses.          |
...
> Status: Active for schema metadata, validation, and resource type generation.
> Schema entries are read and stored in the IR today. Their `path` and `description` fields are active for loading and metadata. A matching resource receives a storage-derived TypeScript definition during generation.
> A schema entry is linked to a resource by key. The TypeScript builder emits the resource definition from the resource's storage metadata and writes it to the planned type-definition path.

Each schema entry describes how an external JSON Schema file participates in the IR.
Schemas can power REST argument validation, UI scaffolding, and generated resource types.

| Path                        | Accepted values & defaults                                                                                                             | CLI & pipeline consumers                | Generated files                                         | WordPress artifacts                                      | Runtime usage                                                       |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------- |
| `schemas.<key>.path`        | Required string path to a JSON Schema file. Relative paths resolve from the config file directory first, then from the workspace root. | Loaded and hashed when building the IR. | Schema JSON copied into the IR for downstream builders. | REST controllers reuse property metadata for validation. | Schema metadata is attached to resources so clients can introspect. |
