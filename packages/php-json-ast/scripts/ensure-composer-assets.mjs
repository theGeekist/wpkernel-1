...
const VENDOR_AUTOLOAD = path.join(PACKAGE_ROOT, 'vendor', 'autoload.php');
const COMPOSER_MANIFEST = path.join(PACKAGE_ROOT, 'composer.json');
...
function execFileAsync(command, args, options) {
	return new Promise((resolve, reject) => {
		execFile(command, args, options, (error, stdout, stderr) => {
			if (error) {
				const message = `${command} ${args.join(' ')} failed: ${stderr || error.message}`;
...
async function ensureComposerAssets() {
	...
	const composerBinary =
		process.env.WPK_PHP_JSON_AST_COMPOSER_BIN ??
		process.env.WPK_CLI_COMPOSER_BIN ??
		'composer';
	const args = [
		'install',
		'--no-dev',
	...
	const { stdout, stderr } = await execFileAsync(composerBinary, args, {
		cwd: PACKAGE_ROOT,
	});
