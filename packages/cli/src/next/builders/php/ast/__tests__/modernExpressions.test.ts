...
function resolveWorkspaceRoot(): string {
	...
	while (true) {
		...
		if (parent === current) {
			throw new Error('Failed to resolve @wpkernel/cli workspace root.');
		}
...
describe('modern PHP expressions', () => {
	...
	it('pretty prints arrow functions, match expressions, and nullsafe calls', () => {
		...
		if (versionCheck.error || versionCheck.status !== 0) {
			...
			expect(arrowStmt.expr?.nodeType).toBe('Expr_Assign');
			expect(
				(
					arrowStmt.expr as typeof arrowStmt.expr & {
						expr?: { nodeType?: string };
					}
				)?.expr?.nodeType
			).toBe('Expr_ArrowFunction');
			expect(callStmt.expr?.nodeType).toBe('Expr_NullsafeMethodCall');
			expect(throwStmt.expr?.nodeType).toBe('Expr_Throw');
		...
		expect(arrowStmt?.expr?.expr?.nodeType).toBe('Expr_ArrowFunction');
		const matchNode = arrowStmt?.expr?.expr?.expr;
