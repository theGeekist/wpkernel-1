...
class WpPostControllerGenerator {
	...
	private readonly canonicalBasePaths: Set<string>;
	...
	public constructor(
		...
		this.canonicalBasePaths = collectCanonicalBasePaths(
			options.routes,
			this.identityParam
		);
...
function collectCanonicalBasePaths(
	...
	const basePaths = new Set(
		routes
			.map(({ route }) =>
				extractCanonicalBasePath(route.path, identityParam)
			)
...
function determineRouteKind(
	...
	const identityMatch = matchesIdentityRoute(
		normalizedPath,
		identityParam,
		canonicalBasePaths
	);
