...
> {
	type PipelineState = AgnosticState<
		...
		TDiagnostic
	>;
	type PausedState = PipelinePaused<PipelineState>;
	...
	const runner = initAgnosticResumableRunner(runnerDependencies);
	...
	const finalizeRun = <TResult>(result: TResult | PausedState) => {
		if (isPaused(result)) {
			return result;
		}
	...
	> = {
		...
		resume(snapshot, resumeInput) {
			try {
				...
				const pendingCheck = waitForPendingExtensionRegistrations();
				const resumeResult = maybeThen(pendingCheck, () => resumeRun());

				if (isPromiseLike(resumeResult)) {
				...
				return finalizeRun(resumeResult);
			} catch (error) {
				diagnosticManager.endRun();
				throw error;
			}
